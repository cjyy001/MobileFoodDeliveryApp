...
class TestRestaurantBrowsing(unittest.TestCase):
    ...
    def test_search_by_cuisine(self):
        ...
        self.assertTrue(all([restaurant['cuisine'] == "Italian" for restaurant in
                             results]))  # Check if all returned restaurants are Italian
    ...
    def test_search_by_location(self):
        ...
        self.assertTrue(all([restaurant['location'] == "Downtown" for restaurant in
                             results]))  # Check if all returned restaurants are in Downtown
    ...
    def test_search_by_rating(self):
        ...
        self.assertTrue(all([restaurant['rating'] >= 4.0 for restaurant in
                             results]))  # Check if all returned restaurants have a rating >= 4.0
    ...
    def test_search_by_cuisine_empty(self):
        """
        Test searching for restaurants by cuisine type when no matches are found.
        """
        results = self.browsing.search_by_cuisine("Nonexistent Cuisine")
        self.assertEqual(len(results), 0)  # There should be no restaurants with the specified cuisine

    def test_search_by_location_empty(self):
        """
        Test searching for restaurants by location when no matches are found.
        """
        results = self.browsing.search_by_location("Nonexistent Location")
        self.assertEqual(len(results), 0)  # There should be no restaurants in the specified location

    def test_search_by_rating_empty(self):
        """
        Test searching for restaurants by minimum rating when no matches are found.
        """
        results = self.browsing.search_by_rating(5.0)  # No restaurant has a rating of 5.0 in the test data
        self.assertEqual(len(results), 0)  # There should be no restaurants with a rating >= 5.0
