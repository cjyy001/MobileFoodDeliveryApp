...
class TestOrderPlacement(unittest.TestCase):
    ...
    def test_validate_order_all_items_available(self):
        """
        Test case for validating an order with all items available in the cart.
        """
        self.cart.add_item("Burger", 8.99, 1)
        self.cart.add_item("Pizza", 12.99, 1)
        self.cart.add_item("Salad", 5.99, 1)
        result = self.order.validate_order()
        self.assertTrue(result["success"])
        self.assertEqual(result["message"], "Order is valid")

    def test_view_cart_after_adding_item(self):
        """
        Test case for viewing the cart after adding an item.
        """
        self.cart.add_item("Pizza", 12.99, 1)
        cart_view = self.cart.view_cart()
        self.assertEqual(len(cart_view), 1)
        self.assertEqual(cart_view[0]["name"], "Pizza")
        self.assertEqual(cart_view[0]["quantity"], 1)
        self.assertEqual(cart_view[0]["subtotal"], 12.99)

    def test_view_cart_after_removing_item(self):
        """
        Test case for viewing the cart after removing an item.
        """
        # 添加几个项目到购物车
        self.cart.add_item("Burger", 8.99, 1)
        self.cart.add_item("Pizza", 12.99, 1)
        ...
        # 确认购物车中只有两个项目（Burger和Salad）
        self.assertEqual(len(cart_view), 2)

        # 确认Pizza已经从购物车中移除
        pizza_in_cart = any(item["name"] == "Pizza" for item in cart_view)
        self.assertFalse(pizza_in_cart)

        # 确认剩余项目的数量和总计正确
        for item in cart_view:
            if item["name"] == "Burger":
                self.assertEqual(item["quantity"], 1)
                self.assertEqual(item["subtotal"], 8.99)
            elif item["name"] == "Salad":
                self.assertEqual(item["quantity"], 1)
                self.assertEqual(item["subtotal"], 5.99)
