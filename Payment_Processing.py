...
class TestPaymentProcessing(unittest.TestCase):
    ...
    def test_process_payment_failure(self):
        ...
        """
        order = {"total_amount": 100.00}
        payment_details = {"card_number": "1111222233334444", "expiry_date": "12/25",
                           "cvv": "123"}  # Simulate a declined card.
    ...
    def test_validate_payment_method_valid_gateway(self):
        """
        Test case for successful validation of a valid payment method ('paypal').
        """
        payment_details = {"paypal_email": "user@example.com"}  # 假设 PayPal 需要邮箱地址
        result = self.payment_processing.validate_payment_method("paypal", payment_details)
        self.assertTrue(result)

    def test_validate_credit_card_valid_details(self):
        """
        Test case for successful validation of valid credit card details.
        """
        payment_details = {"card_number": "1234567812345678", "expiry_date": "12/25", "cvv": "123"}
        result = self.payment_processing.validate_credit_card(payment_details)
        self.assertTrue(result)

    def test_process_payment_with_paypal(self):
        """
        Test case for successful payment processing using the 'paypal' method with valid details.
        """
        order = {"total_amount": 100.00}
        payment_details = {"paypal_email": "user@example.com"}  # 假设 PayPal 需要邮箱地址

        # 使用 mock 来模拟从网关返回的成功支付响应
        with mock.patch.object(self.payment_processing, 'mock_payment_gateway', return_value={"status": "success"}):
